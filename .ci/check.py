...
def js_package():
    artifact_directory()
    js_install()
    # Prove a reused generated directory cannot leak retired license notices.
    (JS / "LICENSES").mkdir(exist_ok=True)
    (JS / "LICENSES/OBSOLETE-LICENSE.txt").write_text("Obsolete generated notice fixture.\n")
    run("npm", "pack", cwd=JS)
    tarball = npm_tarball()
...
if __name__ == "__main__":
    ...
    for command in TOOLS.get(arguments.check, []):
        run(*command)
    if arguments.check in {"metadata", "javascript", "js-package", "js-pnpm", "js-yarn", "js-bun", "python-package", "typst-package"}:
